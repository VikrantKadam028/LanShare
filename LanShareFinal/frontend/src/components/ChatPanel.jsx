...
import { Send, MessageSquare, Lock, Unlock, Image as ImageIcon, X, Check, CheckCheck, ShieldCheck } from 'lucide-react'

// ── Encryption toggle badge ────────────────────────────────────────────────
function EncryptionToggle({ peerId, enabled, onToggle }) {
  return (
    <motion.button
      whileTap={{ scale: 0.93 }}
      onClick={onToggle}
      title={enabled ? 'AES-GCM encryption ON — click to disable' : 'Click to enable end-to-end AES-GCM encryption'}
      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-mono font-semibold border transition-all ${
        enabled
          ...
          : 'bg-white/4 border-white/10 text-ls-muted/50 hover:border-violet-500/25 hover:text-ls-muted'
      }`}
    >
      {enabled
        ? <><ShieldCheck size={11} className="text-emerald-400" /><span>AES-256 ON</span></>
        : <><Unlock size={11} /><span>Encrypt</span></>
      }
    </motion.button>
  )
}
  ...
  if (attach.type === 'image' && attach.data) {
    return (
      <div className="mt-2">
        ...
        {attach.encrypted && (
          <span className="inline-flex items-center gap-0.5 text-[9px] text-emerald-400/60 mt-0.5">
            <ShieldCheck size={8} /> decrypted
          </span>
        )}
      </div>
    )
...
function EncryptedPlaceholder({ msg, onDecrypt }) {
  return (
    <div className="mt-0.5">
      {/* Blurred ciphertext preview */}
      <div className="relative mb-2.5">
        <p className="text-sm font-mono text-ls-muted/60 leading-relaxed break-all select-none"
           style={{ filter: 'blur(4px)', userSelect: 'none', maxWidth: '100%', wordBreak: 'break-all' }}>
          {msg.rawEncrypted?.slice(0, 48) || '🔒••••••••••••••••••••••••••'}
        </p>
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-transparent to-ls-card/60 rounded" />
      </div>
      ...
      <motion.button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-mono font-semibold
                   ...
                   transition-all shadow-[0_0_10px_rgba(16,185,129,0.08)]"
      >
...
// ── Message bubble ────────────────────────────────────────────────────────────
function Bubble({ msg, peerId, onImageClick, onDecrypt }) {
  ...
  return (
    ...
    >
      <div className={`${isMe ? 'msg-sent' : 'msg-recv'} px-3.5 py-2.5 max-w-[78%] ${
        msg.needsDecrypt && !isMe ? 'border border-emerald-500/20 bg-emerald-500/4' : ''
      }`}>
        ...
        {msg.decrypting ? (
          <div className="flex items-center gap-2 py-1">
            ...
            <p className="text-sm text-emerald-400/60 italic font-mono text-[11px]">Decrypting…</p>
          </div>
        ) : msg.needsDecrypt && !isMe ? (
          <EncryptedPlaceholder msg={msg} onDecrypt={() => onDecrypt(msg.id)} />
        ) : (
        ...
        <div className={`flex items-center gap-1 mt-1 ${isMe ? 'justify-end' : ''}`}>
          {(msg.encrypted || msg.wasEncrypted) && (
            <span title="AES-GCM encrypted" className="inline-flex items-center">
              <ShieldCheck size={9} className={isMe ? 'text-emerald-400/60' : 'text-emerald-400/50'} />
            </span>
          )}
          {msg.needsDecrypt && !isMe && (
            <span className="text-[9px] font-mono text-emerald-400/50 flex items-center gap-0.5">
              <Lock size={7} />encrypted
            </span>
          )}
...
export default function ChatPanel() {
  ...
  const [pendingImage, setPending] = useState(null)
  ...
  const encEnabled   = state.encryptedPeers?.[state.selectedPeer] || false
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5 bg-ls-panel/40 flex-shrink-0">
        ...
        {/* Encryption toggle — only for trusted peers */}
        <div className="ml-auto flex items-center gap-2">
          {selectedPeer.trust === 'trusted' && (
            <EncryptionToggle
              peerId={state.selectedPeer}
              enabled={encEnabled}
              onToggle={() => actions.toggleEncryption(state.selectedPeer)}
            />
          )}
          <div className="flex items-center gap-1 text-[10px] font-mono text-ls-muted/40">
      ...
      <AnimatePresence>
        {encEnabled && (
          <motion.div
            ...
            className="bg-emerald-500/8 border-b border-emerald-500/15 px-4 py-1.5 flex items-center gap-2 flex-shrink-0"
          >
            <ShieldCheck size={11} className="text-emerald-400" />
            <span className="text-[10px] text-emerald-400/80 font-mono">
              AES-256-GCM encryption active — messages and images are encrypted end-to-end
            </span>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <AnimatePresence initial={false}>
            {messages.map(msg => (
              <Bubble
                ...
                onDecrypt={(msgId) => actions.decryptMessage(state.selectedPeer, msgId, false)}
              />
            ))}
          </AnimatePresence>
        )}
        ...
        {pendingImage && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <div className="flex-1 min-w-0">
              ...
              {encEnabled && (
                <p className="text-[9px] text-emerald-400/60 flex items-center gap-0.5 mt-0.5">
                  <ShieldCheck size={8} /> will be encrypted
                </p>
              )}
            </div>
      ...
      <div className="border-t border-white/5 p-4 flex-shrink-0">
        <div className={`flex items-end gap-2 bg-ls-card/80 border rounded-xl px-3 py-2.5 transition-colors ${
          encEnabled ? 'border-emerald-500/25 focus-within:border-emerald-500/45' : 'border-white/8 focus-within:border-violet-500/30'
        }`}>
          ...
          <textarea
            ...
            onKeyDown={handleKey}
            placeholder={encEnabled ? '🔒 Encrypted message… (Enter to send)' : 'Type a message… (Enter to send, paste image)'}
            rows={1}
          ...
          >
            {encEnabled ? <ShieldCheck size={14} /> : <Send size={14} />}
          </motion.button>
        </div>
      </div>
