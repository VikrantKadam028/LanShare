...
import { Send, Image as ImageIcon, X, Check, CheckCheck, Users, UserPlus } from 'lucide-react'
...
// System message — shown as a centered pill
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 bg-emerald-500/8 border border-emerald-500/15 rounded-full px-3 py-1">
        <UserPlus size={10} className="text-emerald-400/70 flex-shrink-0" />
        <span className="text-[10px] font-mono text-emerald-400/80">{msg.text}</span>
      </div>
    </motion.div>
  )
}
...
function Bubble({ msg, onImageClick }) {
  ...
  // Don't show system messages in bubble format
...
export default function RoomChat({ room }) {
  ...
  // Filter out duplicate messages by using a Set of message IDs
  const uniqueMessages = React.useMemo(() => {
    const seen = new Set()
    return state.roomMessages.filter(msg => {
        ...
        {uniqueMessages.length === 0 ? (
          ...
          <AnimatePresence initial={false}>
            {uniqueMessages.map(msg => {
              ...
              return <Bubble key={msg.id} msg={msg} onImageClick={setLightbox} />
            })}
          </AnimatePresence>
        )}
        <div ref={endRef} />
      ...
      <AnimatePresence>
        {pending && (
          ...
          >
            ...
            <button onClick={() => setPending(null)} className="text-ls-muted hover:text-rose-400">
              <X size={14} />
            </button>
          </motion.div>
        )}
      ...
      <div className="border-t border-white/5 p-4 flex-shrink-0">
        <div className="flex items-end gap-2 bg-ls-card/80 border border-white/8 rounded-xl px-3 py-2.5 focus-within:border-emerald-500/30 transition-colors">
          <button 
            ...
            className="flex-shrink-0 p-1 text-ls-muted/50 hover:text-emerald-400 transition-colors"
          >
            <ImageIcon size={16} />
          </button>
        ...
        <p className="text-[9px] text-ls-muted/35 font-mono mt-1.5 text-center">
          Sending to all {room.members?.length || 0} members
        </p>
      </div>
