...
async def get_room(code: str, request: Request,
                   from_id: str = None, from_name: str = None,
                   from_ip: str = None, from_port: int = 7734):
    code = code.upper()
    client_ip = _client_ip(request)
    logger.info(f"Room query for {code} from {client_ip} (device: {from_id})")

    # Register the querying peer so we can send messages back to them
    if from_id and from_id != signaling_manager.device_id:
        use_ip = from_ip if from_ip and from_ip not in ("", "127.0.0.1") else client_ip
        signaling_manager.register_peer(from_id, from_name or from_id, use_ip, from_port)
        logger.info(f"Registered querying peer {from_id} ({from_name}) @ {use_ip}:{from_port}")
